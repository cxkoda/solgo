...
func deploy(t *testing.T, totalSupply int64) Interface {
	...
	_, _, nft, err := DeployTestableERC721(sim.Acc(0), sim)
	if err != nil {
		t.Fatalf("DeployTestableERC721(): %v", err)
	}
...
func TestMetadataServer(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			Image: func(_ context.Context, _ Interface, id *TokenID, _ *http.Request, params httprouter.Params) (io.Reader, string, int, error) {
		...
		t.Run(tt.Name, func(t *testing.T) {
			srv := &Server{
				...
				Metadata: []MetadataEndpoint{{
					...
					Handler: func(_ context.Context, _ Interface, id *TokenID, _ *http.Request, params httprouter.Params) (*Metadata, int, error) {
						md := Metadata{
		...
		Metadata: []MetadataEndpoint{
			{
				...
				Handler: func(_ context.Context, _ Interface, id *TokenID, _ *http.Request, _ httprouter.Params) (*Metadata, int, error) {
					md := Metadata{
...
func TestInternalImage(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Empty",
			metadataHandler: func(_ context.Context, _ Interface, id *TokenID, _ *http.Request, _ httprouter.Params) (*Metadata, int, error) {
				md := Metadata{
					Name: fmt.Sprintf("DEFAULT %s", id),
				}
				return &md, 200, nil
...
func TestContextAndRequestPlumbing(t *testing.T) {
	ready := make(chan struct{})
	done := make(chan struct{})

	const hdr = "X-Test-Header"
	...
	srv := &Server{
		Metadata: []MetadataEndpoint{
			{
				Path: "/:tokenId",
				Handler: func(ctx context.Context, _ Interface, _ *TokenID, req *http.Request, _ httprouter.Params) (md *Metadata, httpCode int, err error) {
					got = req.Header.Get(hdr)
					close(ready)
					<-ctx.Done()
					close(done)
					return &Metadata{}, 200, nil
				},
			},
		},
	}
	baseURL := start(t, srv)
	url := fmt.Sprintf("%s/0", baseURL)

	ctx, cancel := context.WithCancel(context.Background())

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		t.Fatalf("http.NewRequestWithContext(ctx, GET, %q, nil) error %v", url, err)
	}

	const want = "A is for apple, B is for butt, C is for cat butt… --Alphabutt by Kimya Dawson"
	req.Header.Set(hdr, want)

	go http.DefaultClient.Do(req)

	<-ready // proves that we've captured the got variable
	if got != want {
		t.Errorf("%T.Metadata[0].Handler got HTTP header %q = %q; want %q", srv, hdr, got, want)
	}

	cancel()
	<-done // proves that the Handler's Context has been closed
}
